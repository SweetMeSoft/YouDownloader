...
public partial class MainWindow : Window
{
    ...
    private async Task Add()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    var videoList = await youtube.Playlists.GetVideosAsync(txtLink.Text);
                    var json = System.Text.Json.JsonSerializer.Serialize(videoList);
        ...
        try
        {
            ...
            if (videoInfo != null && audioStream != null && videoStream != null)
            {
                videoList.Add(new VideoInfo()
                {
                    ...
                    UrlThumbnail = videoInfo.Thumbnails[0].Url[..videoInfo.Thumbnails[0].Url.IndexOf("?")],
                    Video = videoInfo
                });
            }
            else
    ...
    private string GetFilePath(string outputPath, Video videoInfo, int position)
    {
        ...
        var title = videoInfo.Title
            ...
            .Replace("|", "");
        path = position > 0
            ? outputPath + "/[" + videoInfo.UploadDate.ToString("yyyy-MM-dd") + "] " + title + " (" + position + ")"
            : outputPath + "/[" + videoInfo.UploadDate.ToString("yyyy-MM-dd") + "] " + title;

        return File.Exists(path) ? GetFilePath(outputPath, videoInfo, ++position) : path;
    }

    private string GetSize(IStreamInfo audioStream, IStreamInfo videoStream)
    {
        return videoStream.Size.MegaBytes + audioStream.Size.MegaBytes > 1000
            ? (audioStream.Size.GigaBytes + videoStream.Size.GigaBytes).ToString("0.##") + " GB"
            : (audioStream.Size.MegaBytes + videoStream.Size.MegaBytes).ToString("0.##") + " MB";
    }
